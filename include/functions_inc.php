...
function total_news_comments_updater()
{
	$select_user_name = mysql_query("SELECT hosting_user_name FROM hosting_user") or die(mysql_error());
	while($user_name = mysql_fetch_array($select_user_name))
	{
		$select_top_poster_count = mysql_query("SELECT author,COUNT(id_news) FROM comment WHERE author = '{$user_name['hosting_user_name']}' AND id_news  ") or die(mysql_error());
		$top_poster_count = mysql_fetch_array($select_top_poster_count);

		mysql_query("UPDATE hosting_user SET hosting_user_total_news_comments = '{$top_poster_count["COUNT(id_news)"]}' WHERE hosting_user_name = '{$top_poster_count['author']}' ");
	}
}

function total_blog_comments_updater()
{
	$select_user_name = mysql_query("SELECT hosting_user_name FROM hosting_user") or die(mysql_error());
	while($user_name = mysql_fetch_array($select_user_name))
	{
		$select_top_poster_count = mysql_query("SELECT author,COUNT(id_blog) FROM comment WHERE author = '{$user_name['hosting_user_name']}' AND id_blog  ") or die(mysql_error());
		$top_poster_count = mysql_fetch_array($select_top_poster_count);

		mysql_query("UPDATE hosting_user SET hosting_user_total_blog_comments = '{$top_poster_count["COUNT(id_blog)"]}' WHERE hosting_user_name = '{$top_poster_count['author']}' ");
	}
}

function total_forum_comments_updater()
{
	$select_user_name = mysql_query("SELECT hosting_user_name FROM hosting_user") or die(mysql_error());
	while($user_name = mysql_fetch_array($select_user_name))
	{
		$select_top_poster_count = mysql_query("SELECT author,COUNT(id_forum_topic) FROM comment WHERE author = '{$user_name['hosting_user_name']}' AND id_forum_topic  ") or die(mysql_error());
		$top_poster_count = mysql_fetch_array($select_top_poster_count);

		mysql_query("UPDATE hosting_user SET hosting_user_total_forum_comments = '{$top_poster_count["COUNT(id_forum_topic)"]}' WHERE hosting_user_name = '{$top_poster_count['author']}' ");
	}
}

function total_download_comments_updater()
{
	$select_user_name = mysql_query("SELECT hosting_user_name FROM hosting_user") or die(mysql_error());
	while($user_name = mysql_fetch_array($select_user_name))
	{
		$select_top_poster_count = mysql_query("SELECT author,COUNT(id_download) FROM comment WHERE author = '{$user_name['hosting_user_name']}' AND id_download  ") or die(mysql_error());
		$top_poster_count = mysql_fetch_array($select_top_poster_count);

		mysql_query("UPDATE hosting_user SET hosting_user_total_download_comments = '{$top_poster_count["COUNT(id_download)"]}' WHERE hosting_user_name = '{$top_poster_count['author']}' ");
	}
}

/*
function most_online_updater()
{
	...
	while ($most_online = mysql_fetch_array($select_most_online)) 
	{
		...
		while ($most_online_check = mysql_fetch_array($select_most_online_check)) 
		{
			...
			elseif ($date != $most_online_check['hosting_most_online_stamp'] AND $most_online['COUNT(hosting_user_id)'] > $most_online_check['hosting_most_online_record'])			
			{
...
{
	$select_online_status = mysql_query("SELECT * FROM  hosting_user WHERE hosting_user_online_status = 'yes' ") or die(mysql_error());
	if (mysql_numrows($select_online_status) > 0) 
	{
		while ($online_users = mysql_fetch_array($select_online_status))
		{
			...
			$group = mysql_fetch_array($group_check);
				...
				{
					echo "<span style='color:#FC0' title='System Administrator'>".$group['hosting_user_name']."</span> ";				
				}
				
				if ($group['hosting_user_administrator_rights'] == "yes")
				{
					echo "<span style='color:#900' title='Administrator'>".$group['hosting_user_name']."</span> ";
				}
				
				if ($group['hosting_user_moderator_rights'] == "yes")
				{
					echo "<span style='color: #3C0' title='Moderator'>".$group['hosting_user_name']."</span> ";
				}
				
				if ($group['hosting_user_vip_rights'] == "yes")
				{
					echo "<span style='color:#F90' title='VIP'>".$group['hosting_user_name']."</span> ";
				}
				
				if ($group['hosting_user_default_rights'] == "yes")
				{
					echo "<span style='color:#39F' title='Member'>".$group['hosting_user_name']."</span> ";
				}
		}
	}
	else
...
function strip_script($string)
{
	...
	$string = preg_replace("/<iframe[^>]*>.*<*iframe[^>]*>/i", "$type", $string);
	// Prevent linking to source files
	$string = preg_replace("/<iframe[^>]*>/i", "$type", $string);
