...
if ($_SESSION['logged_in']) 
{
	...
	{
		$select_user_profile = mysql_query("SELECT * FROM hosting_user WHERE hosting_user_name = '".$_SESSION['nick']."' ");
		if (mysql_numrows($select_user_profile) > 0) 
		{
			while ($user_profile =  mysql_fetch_array($select_user_profile))
			{
  ...
  <tr>
    ...
    <table width='664' border='0' align='center' cellpadding='0' cellspacing='0'>
        <tr>
          <td width='200' ><center><a href='/?section=edit&id=".$user_profile['hosting_user_id']."&profile=edite&usr_id_edit=".$user_profile['hosting_user_id']."'>{$lang['BODY_PROFILE_EDIT_LINK']}</a></center></td>
        </tr>
        <tr>
          <td><div align='center' ><table width='300' border='0' align='center' cellpadding='0' cellspacing='0' class='ticket'>
      <tr>
        <th scope='col'><img src='{$user_profile['hosting_user_avatar']}'></th>
        </tr>
      <tr>
      ...
      <tr>
        ...
        <th scope='col'>{$user_profile['hosting_user_name']}</th>
        </tr>
      <tr>
        <th scope='col'>{$lang['BODY_PROFILE_JOINED']}:</th>
        <th scope='col'>{$user_profile['hosting_user_join']}</th>
        </tr>
      <tr>
        <th scope='col'>{$lang['BODY_PROFILE_GROUP']}:</th>
        <th scope='col'>";if ($user_profile['hosting_user_rights'] == 2) 
				{
					echo $lang['BODY_PROFILE_GROUP_USER'];
				}
				elseif ($user_profile['hosting_user_rights'] == 6)
				{
					echo $lang['BODY_PROFILE_GROUP_ADMINISTRATOR'];
				}
                echo"</th>
        </tr>
      <tr>
        <th scope='col'>{$lang['BODY_PROFILE_EMAIL']}:</th>
        <th scope='col'><a href='mailto:{$user_profile['hosting_user_email']}'>{$user_profile['hosting_user_email']}</a></th>
      </tr>
      <tr>
        <th scope='col'>{$lang['BODY_PROFILE_STATUS']}:</th>
        <th scope='col'>";
		    if ($user_profile['hosting_user_online_status'] == 'yes') 
		    {
		    	echo $lang['BODY_PROFILE_STATUS_ONLINE'];
		    }
		    elseif ($user_profile['hosting_user_online_status'] == 'no')
		    {
      ...
      <tr>
        ...
        <th scope='col'><a href='{$user_profile['hosting_user_homepage']}'>{$user_profile['hosting_user_homepage']}</a></th>
      </tr>
      <tr>
        <th scope='col'>{$lang['BODY_PROFILE_GENDER']}:</th>
        <th scope='col'>";
		   if ($user_profile['hosting_user_gender'] == 'Male') 
		   {
		   	 echo $lang['BODY_PROFILE_GENDER_MALE'];
		   }
		   elseif ($user_profile['hosting_user_gender'] == 'Female')
		   {
      ...
      <tr>
        ...
        <th scope='col'>{$user_profile['hosting_user_last_time_seen']}</th>
      </tr>
        </table></div></th>
      </tr>
      <tr>
      ...
      <tr>
        <th scope='col'>{$user_profile['hosting_user_signature']}</th>
      </tr>
    </table></td>
        </tr>
    </table></td>
